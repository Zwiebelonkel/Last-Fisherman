...
func show_fish(fish: Dictionary) -> void:
	# 🔒 CRITICAL FIX: Sichere rarity-Zugriffe über FishDB
	...
	# ✅ FIX: Sichere Zugriffe über FishDB
	...
	if is_story_item and rarity == FishDB.RARITY.ANTIK:
		...
		var total_value: int = FishDB.get_fish_value(fish)
...
func _on_continue_pressed() -> void:
	...
	if is_story_item and rarity == FishDB.RARITY.ANTIK:
		...
		if biome != "":
			...
			print("🎭 Story-Item verwendet:", current_fish["id"], "→ Biom:", biome)
