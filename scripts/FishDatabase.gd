...
var RARITY_DATA = {
	...
	RARITY.STORY:         {"name": "???",          "color": Color(0,1,1),     "value": 0.0,  "spawn_chance": 0.0,  "difficulty": 4.0}  # Cyan
}

# ===========================
#  🆕 STORY FISCHE PRO BIOM
# ===========================
var STORY_FISH = {
	"lake": {
		"name": "Der Flüsterer",
		"rarity": RARITY.STORY,
		"base_value": 0,
		"icon": "res://assets/fish/story_lake.png",
		"description": "Seine Schuppen tragen Erinnerungen längst vergessener Zeiten. Er kennt Geheimnisse, die das Wasser vergessen hat.",
		"event_scene": "res://scenes/events/lake_event.tscn",
		"required_catches": 10
	},
	"city": {
		"name": "Neon-Phantom",
		"rarity": RARITY.STORY,
		"base_value": 0,
		"icon": "res://assets/fish/story_city.png",
		"description": "Schwimmt durch digitale Träume und analoge Albträume. Manche sagen, es sei kein Fisch, sondern ein Glitch in der Realität.",
		"event_scene": "res://scenes/events/city_event.tscn",
		"required_catches": 15
	},
	"sewer": {
		"name": "Das Vergessene",
		"rarity": RARITY.STORY,
		"base_value": 0,
		"icon": "res://assets/fish/story_sewer.png",
		"description": "Was die Stadt vergisst, landet hier unten. Manche Dinge schwimmen zurück. Dieses sollte nicht existieren.",
		"event_scene": "res://scenes/events/sewer_event.tscn",
		"required_catches": 20
	},
	"forest": {
		"name": "Wurzelwächter",
		"rarity": RARITY.STORY,
		"base_value": 0,
		"icon": "res://assets/fish/story_forest.png",
		"description": "Älter als die Bäume. Weiser als die Stille. Seine Flossen sind mit Moos bewachsen, seine Augen glühen wie Glühwürmchen.",
		"event_scene": "res://scenes/events/forest_event.tscn",
		"required_catches": 12
	},
	"desert": {
		"name": "Dürre-Orakel",
		"rarity": RARITY.STORY,
		"base_value": 0,
		"icon": "res://assets/fish/story_desert.png",
		"description": "Erinnert sich an Zeiten, als hier noch Ozeane waren. Seine Prophezeiungen sind in Sandkörnern geschrieben.",
		"event_scene": "res://scenes/events/desert_event.tscn",
		"required_catches": 25
	}
}
...
# ===========================
#  🆕 STORY FISH MANAGEMENT
# ===========================
signal story_fish_caught(biome: String, fish: Dictionary)

func get_story_fish(biome: String) -> Dictionary:
	return STORY_FISH.get(biome, {})

func is_story_fish_available(biome: String) -> bool:
	if not STORY_FISH.has(biome):
		return false
	
	# Prüfe ob bereits gefangen
	var story_fish = STORY_FISH[biome]
	if Player.caught_fish_species.get(story_fish["name"], false):
		return false
	
	# Prüfe Fang-Requirement
	var catches_key = "catches_" + biome
	var catches = Player.get(catches_key) if Player.get(catches_key) != null else 0
	return catches >= story_fish["required_catches"]

func try_spawn_story_fish(biome: String) -> Dictionary:
	if is_story_fish_available(biome):
		# 8% Chance wenn verfügbar
		if randf() < 0.08:
			return get_story_fish(biome)
	return {}

# ===========================
#  RANDOM FISH (MIT STORY)
# ===========================
func get_random_from_list(list: Array, bait_level := 1, biome := "") -> Dictionary:
	# 🆕 Story-Fisch Check ZUERST
	if biome != "":
		var story_fish = try_spawn_story_fish(biome)
		if not story_fish.is_empty():
			print("🌟 STORY-FISCH GESPAWNT:", story_fish["name"])
			return story_fish
	...
	return weighted_list.pick_random() if not weighted_list.is_empty() else {}
